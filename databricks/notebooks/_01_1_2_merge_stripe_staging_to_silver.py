...
# COMMAND ----------
# 2.0 
# Merge insert/update silver.stripe_staging_transactions to silver.merged_transactions 
# We do a merge only if there are rows in the staging table
#
# Count rows in stripe_staging_transactions table. 
staging_count = spark.table("hive_metastore.silver.stripe_staging_transactions").count()
# If the count is 0, it means no new files were found.
if staging_count > 0:
    # As stripe_staging_transactions is not empty we can proceed with the merge
    ...
    USING hive_metastore.silver.stripe_staging_transactions AS source
    ON target.transaction_id = source.transaction_id
    WHEN MATCHED THEN UPDATE SET *
    WHEN NOT MATCHED THEN INSERT *
    """)
    # ps: Keeps the table definition, schema, table registered in the metastore
    spark.sql("TRUNCATE TABLE hive_metastore.silver.stripe_staging_transactions")
else:
